...
For the local EBench Generalist adaptation, use the dedicated resumable
setup/4-GPU encode/audit workflow in
[`docs/ebench_lingbot_preprocessing.md`](docs/ebench_lingbot_preprocessing.md).
The canonical artifacts keep each parent episode intact and encode the four
cameras independently. After the full audit, build the lightweight virtual
segment overlay used by training:
...
The overlay enforces `max_self_tokens=81920` without copying or modifying the
canonical latent and action files. It turns 6,600 parent episodes into 7,120
training segments; 520 over-limit parents are split into two segments.
...
# EBench Generalist (full fine-tuning; source and segment audits must pass)
NGPU=4 CONFIG_NAME='ebench_train' bash script/run_va_posttrain.sh

# LIBERO-All with true variable-length episode packing. The global episode
# batch is configurable and must be divisible by NGPU.
NGPU=8 CONFIG_NAME='libero_all_train' bash script/run_va_posttrain.sh \
    --global-episodes-per-update 96 \
    --max-self-tokens 49152 \
    --max-episodes-per-pack 12
```

`ebench_train` packs an effective global batch of 128 virtual segments. Each
segment has at most 193 latent frames (192 real-action blocks), and each
physical pack is limited to 81,920 self-attention tokens and four segments.
Split neighbors share only their boundary anchor latent; their real action
rows do not overlap.

`libero_all_train` combines multiple episodes in one physical sequence up to the configured token budget while a block-diagonal FlexAttention mask prevents cross-episode attention. It still computes an equal-weight mean over `global_episodes_per_update`; changing that parameter changes the optimization batch in the usual way. In packed mode, `batch_size` and fixed `gradient_accumulation_steps` are legacy settings and do not control scheduling. Other training configs retain the original fixed-batch behavior.

#### Recommended LIBERO-All packed settings

The following settings were measured on A100 80 GB GPUs. `max_self_tokens` is
the self-attention token limit of one physical pack on each rank, while
`max_episodes_per_pack` limits how many episodes can enter that pack. The
forward counts are typical values and can vary slightly with episode lengths.

Full fine-tuning:

| GPUs | Global episodes/update | `max_self_tokens` | `max_episodes_per_pack` | Typical forwards/update/rank | Approx. time for 20,000 steps |
| :---: | :---: | :---: | :---: | :---: | :---: |
| 8 | 96 | 49152 | 12 | 1 | 55 hours / 2.3 days |
| 4 | 100 | 98304 | 25 | 1 | 4.5-5 days |
| 4 | 128 | 65536 | 32 | 2 | 6.3-6.5 days |
| 2 | 100 | 24576 | 10 | 6-8 | 9-10 days |

LoRA fine-tuning (rank 64):

| GPUs | Global episodes/update | `max_self_tokens` | `max_episodes_per_pack` | Typical forwards/update/rank | Approx. time for 20,000 steps |
| :---: | :---: | :---: | :---: | :---: | :---: |
| 8 | 96 | 49152 | 12 | 1 | 50 hours / 2.1 days |
| 4 | 100 | 98304 | 25 | 1 | 4.2 days |
| 4 | 128 | 65536 | 32 | 2 | 5.8 days |
| 2 | 100 | 98304 | 32 | 2 | 9 days |

Eight GPUs use a global size of 96 because 100 is not divisible by 8. Four and
two GPUs use 100. Reducing the GPU count therefore does not reduce the global
optimization batch, but each rank must process more episodes. The 2-GPU full
setting intentionally uses the safer 24576-token cap; 32768 approached the
80 GB memory limit in testing. Re-benchmark these limits for other GPU models,
resolutions, sequence-length distributions, or allocator settings.

For global size 128 on four A100 80 GB GPUs, use the 65536-token row rather
than reusing the larger global-100 cap. In a simultaneous 4-GPU LoRA plus
4-GPU Full run, `89984 / 32` reproduced a Full-rank CUDA OOM on the third
update; the remaining ranks later reported an FSDP/NCCL all-gather timeout.
With `65536 / 32`, both jobs completed the corresponding five-update
concurrency test with two physical packs per update. This changes only the
physical forward partition: each optimizer step still averages the same 128
episodes. The first step for a previously unseen pack shape can be much slower
because FlexAttention kernels are compiled; use later steps for throughput
estimates.

For example, launch the recommended 4-GPU jobs with:

```bash
# Full fine-tuning
NGPU=4 CONFIG_NAME='libero_all_train' bash script/run_va_posttrain.sh \
    --save-root /path/to/full-output \
    --global-episodes-per-update 100 \
    --max-self-tokens 98304 \
    --max-episodes-per-pack 25

# LoRA rank-64 fine-tuning
NGPU=4 CONFIG_NAME='libero_all_lora_train' bash script/run_va_posttrain.sh \
    --save-root /path/to/lora-output \
    --global-episodes-per-update 100 \
    --max-self-tokens 98304 \
    --max-episodes-per-pack 25

# Concurrent-safe global-128 Full
NGPU=4 CONFIG_NAME='libero_all_train' bash script/run_va_posttrain.sh \
    --save-root /path/to/full-global-128-output \
    --global-episodes-per-update 128 \
    --max-self-tokens 65536 \
    --max-episodes-per-pack 32

# Concurrent-safe global-128 LoRA
NGPU=4 CONFIG_NAME='libero_all_lora_train' bash script/run_va_posttrain.sh \
    --save-root /path/to/lora-global-128-output \
    --global-episodes-per-update 128 \
    --max-self-tokens 65536 \
    --max-episodes-per-pack 32
```

The reported `step` remains one optimizer update. Therefore, 20,000 steps with
global size 100 process 2,000,000 episode occurrences, whereas 20,000 steps
with global size 80 process 1,600,000. Use 16,000 steps at global size 100 if
the goal is to match the total episode exposure of a global-80, 20,000-step
run.

The LIBERO-All dataset root must contain the collection-level `text_embeddings.pt` produced by `script/prepare_libero_lingbot.py`. Packed loading keeps this small unique-text table on each GPU and memory-maps the per-camera latent records, avoiding reads of their repeated text tensor payloads.

#### LoRA Training

LoRA is opt-in so existing training commands continue to run full fine-tuning. The default LoRA setup uses rank 64, alpha 64, zero dropout, and targets every self/cross-attention Q/K/V/O projection plus both Transformer FFN projections.

The `libero_all_lora_train` config additionally trains `action_embedder`,
`condition_embedder_action`, and `action_proj_out` in full. These modules are
stored inside the PEFT adapter checkpoint and use the full-model optimizer
scale (`lr=1e-5`, `weight_decay=1e-1`), while LoRA parameters retain
`lr=1e-4` and `weight_decay=1e-2`.

```bash
# Start adapter-only LoRA training
NGPU=8 CONFIG_NAME='libero_train' bash script/run_va_posttrain.sh \
    --training-mode lora

# LIBERO-All packed LoRA. See the tables above for settings by GPU count.
NGPU=8 CONFIG_NAME='libero_all_lora_train' bash script/run_va_posttrain.sh \
    --global-episodes-per-update 96 \
    --max-self-tokens 49152 --max-episodes-per-pack 12

# Resume adapter, optimizer, scheduler, sampler, and RNG state
NGPU=8 CONFIG_NAME='libero_train' bash script/run_va_posttrain.sh \
    --training-mode lora \
    --resume-from train_out/checkpoints/checkpoint_step_200
```

Each LoRA checkpoint contains an `adapter/` directory and `trainer_state/`; it does not contain the base Transformer weights. Resume with the same dataset and base model. Changing GPU count or batch topology is supported, but the resumed random/data trajectory will no longer be bit-exact.

Load an adapter for inference by keeping the original model root in the selected config and supplying the base Transformer and adapter paths:

```bash
python -m torch.distributed.run --nproc_per_node=1 -m wan_va.wan_va_server \
    --config-name libero \
    --transformer-path /path/to/base/model/transformer \
    --lora-adapter-path train_out/checkpoints/checkpoint_step_200/adapter
```
