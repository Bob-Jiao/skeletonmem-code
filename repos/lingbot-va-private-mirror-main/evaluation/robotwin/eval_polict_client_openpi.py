...
robowin_root = Path(
    os.environ.get("ROBOTWIN_ROOT", "/root/zouyude/RoboTwin-lingbot-va")
).expanduser().resolve()
if not robowin_root.is_dir():
    raise FileNotFoundError(
        f"RoboTwin root does not exist: {robowin_root}. "
        "Set ROBOTWIN_ROOT to the pinned RoboTwin checkout."
    )
...
def main(usr_args):
    ...
    save_videos = read_bool_env("SAVE_VIDEOS", default=True)
    args["save_videos"] = save_videos
    args["eval_video_log"] = bool(args.get("eval_video_log", False)) and save_videos
    ...
    print("\033[94mSave Evaluation Videos:\033[0m " + str(save_videos))
    print("\n==================================")
