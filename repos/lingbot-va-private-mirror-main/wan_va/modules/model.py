...
class FlexAttnFunc(nn.Module):
    ...
    def init_mask_from_metadata(
        ...
        """Build block-diagonal masks for an episode-major packed sequence."""
        torch._inductor.config.realize_opcount_threshold = 100
        ...
        window_sizes = window_sizes.long().to(device)

        mask_mod = FlexAttnFunc._get_mask_mod(
            seq_ids, frame_ids, noise_ids, window_sizes
        )
    ...
    def _get_mask_mod(seq_ids, frame_ids, noise_ids, window_size):
        variable_windows = torch.is_tensor(window_size)
        ...
        def block_window_mask(
            ...
            current_window = (
                window_size[seq_ids[q_idx].clamp_min(0)]
                if variable_windows
                else window_size
            )
            return ((frame_ids[q_idx] - frame_ids[kv_idx]).abs() <= current_window)
        ...
        mask = and_masks(mask, block_window_mask)
        return mask
    ...
    def __init__(
        ...
        if attn_mode == 'torch':
            ...
            if flash_attn_func is None:
                raise ImportError(
                    "Flash Attention mode requires `flash_attn` or "
                    "`flash_attn_interface` to be installed."
                )
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def _forward_train_packed(self, input_dict):
        """Run multiple variable-length episodes as one isolated super-sequence."""
        latent_dict = input_dict["latent_dict"]
        action_dict = input_dict["action_dict"]
        for values in (latent_dict, action_dict):
            values["noisy_latents"] = values["noisy_latents"].to(torch.bfloat16)
            values["latent"] = values["latent"].to(torch.bfloat16)

        if latent_dict["noisy_latents"].shape[0] != 1:
            raise ValueError("Packed training requires a physical batch dimension of one")
        if self.patch_size[0] != 1:
            raise ValueError("Packed training currently requires patch_size[0] == 1")

        frame_lengths = [int(value) for value in input_dict["frame_lengths"].tolist()]
        chunk_sizes = [int(value) for value in input_dict["chunk_sizes"].tolist()]
        window_sizes = input_dict["window_sizes"]
        if not frame_lengths or len(chunk_sizes) != len(frame_lengths):
            raise ValueError("Packed frame/chunk metadata is inconsistent")
        if sum(frame_lengths) != latent_dict["noisy_latents"].shape[2]:
            raise ValueError("Packed frame lengths do not match the latent tensor")
        if sum(frame_lengths) != action_dict["noisy_latents"].shape[2]:
            raise ValueError("Packed frame lengths do not match the action tensor")

        latent_noisy = self._input_embed(
            latent_dict["noisy_latents"], input_type="latent"
        )
        latent_clean = self._input_embed(latent_dict["latent"], input_type="latent")
        action_noisy = self._input_embed(
            action_dict["noisy_latents"], input_type="action"
        )
        action_clean = self._input_embed(action_dict["latent"], input_type="action")
        ...
        latent_noisy_temb, latent_noisy_proj = self._time_embed(
        ...
        latent_clean_temb, latent_clean_proj = self._time_embed(
        ...
        action_noisy_temb, action_noisy_proj = self._time_embed(
        ...
        action_clean_temb, action_clean_proj = self._time_embed(
        ...
        latent_grid = latent_dict["grid_id"]
        action_grid = action_dict["grid_id"]
        ...
        frame_id_parts = []
        noise_id_parts = []
        latent_output_slices = []
        action_output_slices = []
        packed_offset = 0
        frame_offset = 0

        for seq_id, (frame_count, chunk_size) in enumerate(
            zip(frame_lengths, chunk_sizes)
        ):
            latent_start = frame_offset * video_tokens_per_frame
            latent_end = (frame_offset + frame_count) * video_tokens_per_frame
            action_start = frame_offset * action_tokens_per_frame
            action_end = (frame_offset + frame_count) * action_tokens_per_frame
            latent_length = latent_end - latent_start
            action_length = action_end - action_start

            episode_hidden = [
                latent_noisy[:, latent_start:latent_end],
                latent_clean[:, latent_start:latent_end],
                action_noisy[:, action_start:action_end],
                action_clean[:, action_start:action_end],
            ]
            episode_grid = [
                latent_grid[:, :, latent_start:latent_end],
                latent_grid[:, :, latent_start:latent_end],
                action_grid[:, :, action_start:action_end],
                action_grid[:, :, action_start:action_end],
            ]
            episode_temb = [
                latent_noisy_temb[:, latent_start:latent_end],
                latent_clean_temb[:, latent_start:latent_end],
                action_noisy_temb[:, action_start:action_end],
                action_clean_temb[:, action_start:action_end],
            ]
            episode_proj = [
                latent_noisy_proj[:, latent_start:latent_end],
                latent_clean_proj[:, latent_start:latent_end],
                action_noisy_proj[:, action_start:action_end],
                action_clean_proj[:, action_start:action_end],
            ]
            episode_length = 2 * (latent_length + action_length)
            padded_length = (128 - episode_length % 128) % 128

            hidden_parts.extend(episode_hidden)
            grid_parts.extend(episode_grid)
            temb_parts.extend(episode_temb)
            timestep_proj_parts.extend(episode_proj)

            local_latent_frames = torch.arange(
                frame_count, device=latent_noisy.device
            ).repeat_interleave(video_tokens_per_frame)
            local_action_frames = torch.arange(
                frame_count, device=latent_noisy.device
            ).repeat_interleave(action_tokens_per_frame)
            latent_frame_ids = (local_latent_frames // chunk_size) * 2
            action_frame_ids = (local_action_frames // chunk_size) * 2 + 1
            frame_id_parts.extend(
                [latent_frame_ids, latent_frame_ids, action_frame_ids, action_frame_ids]
            )
            seq_id_parts.append(
                torch.full(
                    (episode_length,), seq_id, dtype=torch.long, device=latent_noisy.device
                )
            )
            noise_id_parts.append(
                torch.cat(
                    [
                        torch.zeros(latent_length, device=latent_noisy.device),
                        torch.ones(latent_length, device=latent_noisy.device),
                        torch.zeros(action_length, device=latent_noisy.device),
                        torch.ones(action_length, device=latent_noisy.device),
                    ]
                )
            )

            latent_output_slices.append((packed_offset, packed_offset + latent_length))
            action_output_start = packed_offset + 2 * latent_length
            action_output_slices.append(
                (action_output_start, action_output_start + action_length)
            )
            packed_offset += episode_length

            if padded_length:
                hidden_parts.append(
                    latent_noisy.new_zeros((1, padded_length, latent_noisy.shape[-1]))
                )
                grid_parts.append(
                    latent_grid.new_zeros((1, latent_grid.shape[1], padded_length))
                )
                temb_parts.append(
                    latent_noisy_temb.new_zeros(
                        (1, padded_length, latent_noisy_temb.shape[-1])
                    )
                )
                timestep_proj_parts.append(
                    latent_noisy_proj.new_zeros(
                        (1, padded_length, *latent_noisy_proj.shape[2:])
                    )
                )
                seq_id_parts.append(
                    torch.full(
                        (padded_length,), -1, dtype=torch.long, device=latent_noisy.device
                    )
                )
                frame_id_parts.append(
                    torch.full(
                        (padded_length,), -1, dtype=torch.long, device=latent_noisy.device
                    )
                )
        ...
        if not (
            ...
            raise RuntimeError("Packed token metadata length mismatch")
        planned_tokens = int(input_dict.get("planned_tokens", hidden_states.shape[1]))
        if hidden_states.shape[1] != planned_tokens:
            raise RuntimeError(
                f"Planner/model token mismatch: planned={planned_tokens}, "
                f"actual={hidden_states.shape[1]}"
            )

        rotary_emb = self.rope(full_grid_id)[:, :, None]
        text_seq_ids = torch.arange(
            len(frame_lengths), device=hidden_states.device
        ).repeat_interleave(text_length)
        FlexAttnFunc.init_mask_from_metadata(
            seq_ids,
            frame_ids,
            noise_ids,
            text_seq_ids,
        ...
        latent_hidden_states = torch.cat(
            [hidden_states[:, start:end] for start, end in latent_output_slices], dim=1
        )
        action_hidden_states = torch.cat(
            [hidden_states[:, start:end] for start, end in action_output_slices], dim=1
        )
