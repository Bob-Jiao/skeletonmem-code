...
class VA_Server:
    ...
    def __init__(self, job_config):
        ...
        self.save_debug_data = getattr(job_config, 'save_debug_data', False)
        ...
        transformer_path = getattr(job_config, 'transformer_model_name_or_path', None)
        ...
        self.transformer = load_transformer(
            ...
            lora_adapter_path=getattr(job_config, 'lora_adapter_path', None),
            attn_mode="torch"
        )
    ...
    def _initial_action_condition(self, actions):
        """Match the raw-zero prefix inserted by the training dataset."""
        raw_zero_action = np.zeros(
            (
        ...
        return self.preprocess_action(raw_zero_action).to(actions)
    ...
    def _infer(self, obs, frame_st_id=0):
        ...
        with (
            ...
            action_cond = (
                self._initial_action_condition(actions)
                if frame_st_id == 0
                else None
            )
            if action_cond is not None:
                logger.info(
                    "Initial action condition (used channels): %s",
                    action_cond[
                        0, self.job_config.used_action_channel_ids, 0, 0, 0
                    ].float().cpu().tolist(),
                )
    ...
    def infer(self, obs):
        ...
        if reset:
            ...
            if return_video:
                # Qualitative evaluation only: expose the model's native decoded
                # rollout.  Normal action-only clients never enter this branch.
                ...
                decoded_video = np.asarray(self.decode_one_video(latents, 'np')[0])
                if np.issubdtype(decoded_video.dtype, np.floating):
                    decoded_video = np.clip(decoded_video * 255.0, 0.0, 255.0)
                result['video'] = decoded_video.astype(np.uint8)
            return result
...
def run(args):    
    ...
    config.save_debug_data = args.save_debug_data
    if args.transformer_path is not None:
        config.transformer_model_name_or_path = args.transformer_path
...
def main():
    ...
    parser.add_argument(
        "--save-debug-data",
        action="store_true",
        help="Save observation, video-latent, and raw-action tensors for debugging",
    )
    parser.add_argument(
        "--transformer-path",
        type=str,
        default=None,
        help="Optional base transformer directory; other model components still use the configured root.",
    )
    parser.add_argument(
        "--lora-adapter-path",
        type=str,
        default=None,
        help="Optional LoRA adapter directory to load on top of the base transformer.",
    )
